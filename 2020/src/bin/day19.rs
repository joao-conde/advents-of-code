...
fn main() {
    ...
    rules.insert(
        "8".to_owned(),
        "42 | 42 42 | 42 42 42 | 42 42 42 42 | 42 42 42 42 42".to_owned(),
    );
    rules.insert(
        "11".to_owned(),
        "42 31 | 42 42 31 31 | 42 42 42 31 31 31 | 42 42 42 42 31 31 31 31".to_owned(),
    );
...
fn expand(rules: &Rules, rule: String) -> String {
    let tokens = rules
        .get(&rule)
        .unwrap_or_else(|| unreachable!("no such rule {}", rule));
