const input = require("fs").readFileSync("input/day09").toString();

const MULTIPLE = 23;
const numMarbles = parseInt(input.match("[0-9]*(?= points)")[0]);
...
const computeElfScore = (numPlayers, numMarbles) => {
    ...
    let curPlayer = 0;
    let curMarble = { val: 0, prev: null, next: null };
    curMarble.next = curMarble;
    curMarble.prev = curMarble;
    for (let marbleToAdd = 1; marbleToAdd <= numMarbles; marbleToAdd += 1) {
        if (marbleToAdd % MULTIPLE !== 0) {
            ...
            for (let i = 0; i < 7; i += 1) toRemove = toRemove.prev;
