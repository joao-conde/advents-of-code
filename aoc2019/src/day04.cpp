...
bool isValidPassword(int password, bool exactReplication){
    ...
    while(!ss.eof()){
        ...
        if(exactReplication){
            if(atoi(&mult) == 2) doubleFound = true; //exact double rule
        }
        else{
            if(atoi(&mult) >= 2) doubleFound = true; //"lazy" double rule
        }
        if(letter > nextLetter && nextLetter != EOF) return false; //ever increasing order rule
    }
    return doubleFound;
}
