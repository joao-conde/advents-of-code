...
states = (
    {}
)  # state to (0: (write value, movement delta, next state), 1: (write value, movement delta, next state))
...
while len(input_lines) > 0:
    ...
    move_delta = (
        1 if re.findall(re_move_direction, cur_state_lines[3])[0] == "right" else -1
    )
    ...
    move_delta = (
        1 if re.findall(re_move_direction, cur_state_lines[7])[0] == "right" else -1
    )
