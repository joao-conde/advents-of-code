...
fn parse_input(input: &str) -> BagRules {
    ...
    let contents_re =
        Regex::new(r"(?P<quantity>[0-9]+) (?P<bag>\w+ \w+)").expect("invalid regex expression");
    ...
    for line in input.lines() {
        ...
        for capture in contents_re.captures_iter(line) {
            ...
            let quantity = capture["quantity"]
                .parse()
                .expect("invalid quantity, must be unsigned int");
            rules
                .entry(bag.clone())
                .or_insert_with(Vec::new)
                .push((quantity, child_bag));
        }
    }
    rules
}
