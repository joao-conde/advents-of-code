...
fn main() {
    ...
    let (loop_tiles, p1) = find_loop_tiles(&tiles);
    println!("Part1: {p1}");

    let p2: usize = tiles
        .iter()
        .enumerate()
        .map(|(i, _)| count_enclosed_tiles(&tiles, &loop_tiles, i))
        .sum();
    println!("Part2: {p2}");
}

fn find_loop_tiles(tiles: &[Vec<char>]) -> (HashSet<(usize, usize)>, usize) {
    ...
    // field dimensions and starting point
    let nrows = tiles.len();
    ...
    while let Some((i, j, distance)) = deque.pop_front() {
        ...
        let cur_tile = tiles[i][j];

        if i > 0 && north_conns.contains(&cur_tile) && south_conns.contains(&tiles[i - 1][j]) {
            deque.push_back((i - 1, j, distance + 1));
        }

        if i + 1 < nrows
            && south_conns.contains(&cur_tile)
            && north_conns.contains(&tiles[i + 1][j])
        {
            deque.push_back((i + 1, j, distance + 1));
        }

        if j > 0 && west_conns.contains(&cur_tile) && east_conns.contains(&tiles[i][j - 1]) {
            deque.push_back((i, j - 1, distance + 1));
        }

        if j + 1 < ncols && east_conns.contains(&cur_tile) && west_conns.contains(&tiles[i][j + 1])
        {
            deque.push_back((i, j + 1, distance + 1));
        }
    }
...
fn count_enclosed_tiles(
    ...
    for (j, &tile) in tiles[i].iter().enumerate() {
